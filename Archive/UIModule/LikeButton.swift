...
class LikeButton: UIView {
  ...
  private var isActiveCompleteAction: Bool = false
  ...
  private func likeAnimation(completion: @escaping () -> Void) {
    self.likeLottieView.play(completion: { [weak self] isDone in
      if self?.isActiveCompleteAction ?? false {
        completion()
      }
    })
  }
  ...
  private func unlikeAnimation(completion: @escaping () -> Void) {
    self.likeLottieView.play(fromFrame: 29.0, toFrame: 1.0, loopMode: .playOnce, completion: { [weak self] isDone in
      if self?.isActiveCompleteAction ?? false {
        completion()
      }
    })
  ...
  @objc private func likeClicked() {
    self.isActiveCompleteAction = true
    haptic()
  ...
  func stopAnimation() {
    self.isActiveCompleteAction = false
    self.likeLottieView.stop()
  }
